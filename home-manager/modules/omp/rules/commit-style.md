...
ceiling. Add a body only when the change hides a decision the diff cannot
show, keep it to a single line, and pass it inside one `-m`: repeated
`-m` flags are refused. The message is raw literal text and nothing else:
