...
| Router-side DPI bypass, zapret exclude list (not nix-managed) | `docs/dpi-bypass.md` | The ISP filters TLS by exact SNI: the same address answers or hangs on the name in the ClientHello alone, while TCP always opens. zapret's generic `--filter-tcp=80,443` profile desyncs everything except `/opt/zapret/ipset/zapret-hosts-user-exclude.txt`, so a name in that list gets no bypass — which is why `cloudfront.net` sitting there left Duolingo's three audio distributions silent while its API worked. Removed 2026-09-26, verified by handshake timing before and after, collateral checked across seven CloudFront-fronted sites. `aws.amazon.com` is still dead through the `amazon.com` entry, deliberately untouched |
