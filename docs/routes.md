...
| Swap: zram hot tier only (`server`; NVMe overflow tier removed temporarily), `vm` sysctls (imported by `hosts/configuration.nix` and `hosts/configuration-server.nix`) | `modules/swap.nix` | The 32 GiB box ran with no swap at all and took 70 kernel OOM kills in 9 days of uptime (steamwebhelper, chrome, electron): `pgsteal_anon` was 0 while 72M file pages refaulted, so every byte of reclaim pressure landed on the page cache. `zram0` is `zstd` sized at 50% of RAM, `swap-priority` 100. `modules/swap.nix` sets the shared default, one `swapDevices` entry at `/swapfile`, 32 GiB, `priority` 10 — this is what `pc` keeps, still on its NVMe. The 16 GiB `/swapfile` was removed from `hosts/configuration-server.nix` (via `lib.mkForce [ ]`) because swap-out traffic heats the NVMe beyond thermal throttle points: observed 74 °C during `cargo build` with swap writes. Until a dedicated SATA SSD is added for swap (outside the ZFS mirror, which has an OpenZFS deadlock with swap), memory pressure beyond zram capacity triggers the OOM killer instead of disk swap. The thermal throttle thresholds are already set to 70 °C (light) and 80 °C (heavy) via HCTM.
| Early OOM killing: `earlyoom` prefers killing expendable build tools (compilers/linkers) over the tmux/omp session before kernel OOM-thrash triggers `DefaultOOMPolicy=stop`; `systemd.oomd` disabled to avoid fighting it (imported by `hosts/configuration.nix` and `hosts/configuration-server.nix`, same hosts as `modules/swap.nix`) | `modules/oom.nix` | `DefaultOOMPolicy=continue` for both the system and user managers lives in `modules/systemd.nix` (all hosts via `modules/base.nix`) so a kernel OOM kill never stops the whole scope/session |
...
| Drive mounts (`/drive` ZFS pool `data`, mirror of a 2 TB SATA SSD + a 2 TB SMR HDD; `/torrents` 4 TB SATA HDD; torrent bind mount) | `modules/drive.nix`; pool details: `docs/zfs-pool.md` | `/drive`'s datasets mount via their own ZFS `mountpoint` property (`zfs-mount.service`), not `fileSystems` entries — the pool was created live with native mountpoints already set and changing them to `legacy` would require a `zfs set` against the constraint that this migration touch no live pool state. `boot.zfs.extraPools = [ "data" ]` makes NixOS import it every boot regardless of `/etc/zfs/zpool.cache` state (the pool was created outside a rebuild). Because `/drive` therefore has no systemd mount unit, anything bind-mounting into it (`/drive/torrents` here; `/var/lib/private/chromadb` in `modules/chromadb.nix`) needs an explicit `x-systemd.requires=zfs-mount.service` mount option — the automatic nested-mount-point ordering systemd derives from declared `fileSystems` entries does not see a native ZFS mount. The torrent bind also carries `x-systemd.requires-mounts-for=/torrents` (source) and `nofail` (don't block boot if `/torrents` is late); bind mount targets need no `systemd.tmpfiles` rule because systemd creates a missing bind-mount target directory itself. The NFS export gained `crossmnt` so children of `/drive` — now separate ZFS filesystems plus the bind — are reachable without one export line per dataset. `/drive/torrents_db` was a same-era bind for a second, always-empty directory; it was removed on 2026-09-22 (see `docs/zfs-pool.md`) once qBittorrent's resume data was confirmed to live at `/var/lib/qBittorrent/qBittorrent/data/BT_backup` on the NVMe root instead. `/drive` must never carry `_netdev`: it is a local disk, but `_netdev` orders its mount after `network.target`, and a bind mount of one of its subdirectories into `local-fs.target` previously closed a cycle through `sockets.target`. This cycle was present on every boot from the chromadb bind mount landing (`chromadb: keep index off the system nvme`, 2026-09-04) until the ext4 `/drive` was replaced by this ZFS pool — it is not specific to kernel 6.18.41. systemd broke it by deleting an arbitrary job, and which job varied by boot: on 2026-09-21 it deleted `nix-daemon.socket`, leaving that boot without a nix daemon; earlier boots (kernel 7.1.5) lost `systemd-update-utmp`, `dbus-broker`, `systemd-tmpfiles-setup`… |
...
| Router-side DPI bypass, zapret exclude list (not nix-managed) | `docs/dpi-bypass.md` | The list breaks apps in both directions. Holding a name the ISP SNI-filters means no bypass and a hanging handshake: `cloudfront.net` sat there and left the Duolingo web CDNs dead, removed 2026-09-26 with collateral checked across seven CloudFront-fronted sites. Missing a name that cannot survive desync means the opposite — the Android app fetches audio from `tts-static.duolingo.cn` on Alibaba, which answers a split ClientHello with plaintext `HTTP/1.1 400` (`WRONG_VERSION_NUMBER`, not a timeout), so `duolingo.cn` was added next to `duolingo.com` the same day. `/tmp/zapret+nfqws+3+main.log` prints the exclude verdict and the hostnames clients really request; `aws.amazon.com` is still dead through the `amazon.com` entry, deliberately untouched |
