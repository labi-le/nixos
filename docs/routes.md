...
| Swap: zram hot tier, HDD overflow tier (`server` only), `vm` sysctls (imported by `hosts/configuration.nix` and `hosts/configuration-server.nix`) | `modules/swap.nix` | The 32 GiB box ran with no swap at all and took 70 kernel OOM kills in 9 days of uptime (steamwebhelper, chrome, electron): `pgsteal_anon` was 0 while 72M file pages refaulted, so every byte of reclaim pressure landed on the page cache. `zram0` is `zstd` sized at 50% of RAM, `swap-priority` 100. `modules/swap.nix` sets the shared default, one `swapDevices` entry at `/swapfile`, 32 GiB, `priority` 10 — this is what `pc` keeps, still on its NVMe. `hosts/configuration-server.nix` overrides it with `lib.mkForce` (list options concatenate rather than replace, so a plain second entry would leave both `/swapfile` and `/backup/swapfile` active on `server`) to a single entry on the 4 TB SATA HDD at `/backup/swapfile` (`modules/drive.nix`), same size and priority, plus `options = [ "nofail" ]`. The overflow tier exists as a fail-safe to prevent the OOM killer (the 70-kill incident shows the cost); it is an overflow-of-last-resort, not a performance tier — this is an SMR disk, and SMR random-write latency runs around 10 ms per operation, orders of magnitude past an NVMe's tail latency, so any workload that actually reaches this tier will crawl. Under `vm.swappiness = 150` (Linux 5.8+ weights anonymous reclaim above file reclaim) a workload that saturates zram and then leans on this tier can stall in uninterruptible sleep waiting on 10 ms SMR writes instead of reaching the OOM killer — a known risk, left unfixed. `systemd.oomd.enableSystemSlice` was considered as a mitigation and deliberately rejected: it changes the kill policy for every service on both hosts and is a separate decision, not bundled into this migration. Moving swap off the server's NVMe was precautionary, not a fix for a measured swap problem: swap held 0 B of the 32 GiB tier for the whole window the NVMe reported 62°C and 8.33 TB written (10% of rated endurance) at ~297 MB/min — `/proc/diskstats` plus per-process `write_bytes` attributed that write volume to the agent harness writing under `$HOME`, not to swap, and SMART showed zero thermal-throttle transitions (`Thermal Management T1/T2 Trans Count = 0`) and zero `Warning Comp. Temperature Time`, i.e. 62°C was not causing throttling. Relocating the 32 GiB worst-case swap write source off the NVMe removes a plausible future contributor without claiming it caused what was measured; `pc` is unaffected; its NVMe still carries no swap tier concern documented here. `/backup` is a permanent boot-time mount (not an automount): a swap unit's `RequiresMountsFor` needs the mount to be scheduled at boot, not racing its on-demand trigger. `nofail` is required, not cosmetic: without it the plain `defaults` options put the generated swap unit in `swap.target.requires/`, and `swap.target` is ordered `Before=sysinit.target`, so a missing or still-`fsck`ing 4 TB disk would serialise the entire boot — including sshd — behind it on a headless machine; `nofail` demotes that to `swap.target.wants/`. Maintenance on `/backup` must stop `backup-swapfile.swap` first: once the swapfile is active it pins the mount busy, and `systemctl restart backup.mount` fails with EBUSY and drags `drive-torrents.mount`, `drive-torrents_db.mount` and qbittorrent down with it through `Requires=` propagation. A superseded swapfile (path, size, or host reassignment) is only `swapoff`ed by activation, never deleted — the file itself must be removed by hand or it sits as dead weight on disk. On the `pc` host, `zswap.enabled=0` because the CachyOS kernel ships `CONFIG_ZSWAP_DEFAULT_ON` and zswap in front of a zram device compresses every page twice. On the `server` host running `pkgs.linuxPackages`, the kernel has `ZSWAP = option yes` with no `ZSWAP_DEFAULT_ON`, making the `zswap.enabled` parameter inert. `vm.swappiness` needs `lib.mkForce`: musnix (`audio.lowLatency`) defines it as 10 without `mkDefault`, and 10 keeps the kernel from using zram at all. `vm.watermark_scale_factor` 125 keeps kswapd reclaim tighter; `vm.watermark_min_kbytes` is not set. No manual `systemd.services`/`systemd.swap` ordering was needed: nixpkgs' own `nixos/modules/config/swap.nix` already sets `unitConfig.RequiresMountsFor = [ (dirOf sw.device) ]` on the generated `mkswap-<name>.service`, and `systemd.swap(5)` documents that a swap unit backed by a file gets an automatic `Requires=`/`After=` on the mount unit of the filesystem it lives on — both fire correctly the moment `/backup` is an ordinary fstab mount instead of an automount. |
...
| Drive mounts (`/drive` ZFS pool `data`, mirror of a 2 TB SATA SSD + a 2 TB SMR HDD; `/backup` 4 TB SATA HDD; torrent bind mounts) | `modules/drive.nix`; pool details: `docs/zfs-pool.md` | `/drive`'s datasets mount via their own ZFS `mountpoint` property (`zfs-mount.service`), not `fileSystems` entries — the pool was created live with native mountpoints already set and changing them to `legacy` would require a `zfs set` against the constraint that this migration touch no live pool state. `boot.zfs.extraPools = [ "data" ]` makes NixOS import it every boot regardless of `/etc/zfs/zpool.cache` state (the pool was created outside a rebuild). Because `/drive` therefore has no systemd mount unit, anything bind-mounting into it (`/drive/torrents`, `/drive/torrents_db` here; `/var/lib/private/chromadb` in `modules/chromadb.nix`) needs an explicit `x-systemd.requires=zfs-mount.service` mount option — the automatic nested-mount-point ordering systemd derives from declared `fileSystems` entries does not see a native ZFS mount. The torrent binds also carry `x-systemd.requires-mounts-for=/backup` (source) and `nofail` (don't block boot if `/backup` is late); bind mount targets need no `systemd.tmpfiles` rule because systemd creates a missing bind-mount target directory itself. The NFS export gained `crossmnt` so children of `/drive` — now separate ZFS filesystems plus the two binds — are reachable without one export line per dataset. `/drive` must never carry `_netdev`: it is a local disk, but `_netdev` orders its mount after `network.target`, and a bind mount of one of its subdirectories into `local-fs.target` previously closed a cycle through `sockets.target`. This cycle was present on every boot from the chromadb bind mount landing (`chromadb: keep index off the system nvme`, 2026-09-04) until the ext4 `/drive` was replaced by this ZFS pool — it is not specific to kernel 6.18.41. systemd broke it by deleting an arbitrary job, and which job varied by boot: on 2026-09-21 it deleted `nix-daemon.socket`, leaving that boot without a nix daemon; earlier boots (kernel 7.1.5) lost `systemd-update-utmp`, `dbus-broker`, `systemd-tmpfiles-setup`… |
