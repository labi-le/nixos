...
| DoH public endpoint: vhost, rate limit, response hardening | `modules/nginx.nix` | Vhost `dns.labile.cc` on Angie 443 with `enableACME`; `location = /dns-query` (exact match on purpose — a prefix location proxies `/dns-queryZZZ` to the daemon and its backend 404s feed the `nginx-scan-404` fail2ban jail) proxies to `127.0.0.1:8053`, `location /` returns 404, `limit_req zone=doh` (60 r/min per IP, burst 120) and `log_format doh` live in `commonHttpConfig`. The endpoint is deliberately open — no token, no client restriction; the rate limit is the only access control and must live at nginx because DoH arrives at unbound from 127.0.0.1, and it bounds one abuser per address, not a distributed one, which is why sustained rejection also feeds the `nginx-doh-abuse` jail. `proxy_hide_header` strips `Access-Control-Allow-Origin` (doh-server sets `*`, which would let any web page query this resolver through its visitors) and `X-Powered-By` (names the exact build). The location logs with `log_format doh`, the combined format minus `$query_string`; the Alloy `stage.replace` in `modules/grafana.nix` redacts what still slips through `error.log` |
| DNS query logging and outbound rate limit | `modules/unbound.nix` | `log-replies = true` without `log-queries`: the reply line already carries client, qname, type, class, rcode, timing and size, so the query line is the same data at twice the journal volume. `verbose = true` on `services.doh-server` logs each DoH request, but `log_guessed_client_ip` is deliberately absent — it reads the first global address out of `X-Forwarded-For`, which the client sets and nginx appends to rather than replaces, so an outsider could forge "IP X looked up Y" log lines; without it the daemon logs `X-Real-IP`, which nginx does replace. unbound sees every DoH client as `127.0.0.1`, so per-client attribution needs the nginx access log correlated by timestamp. `ratelimit = 1000` caps outbound queries per target zone, the only applicable control since `ip-ratelimit` cannot distinguish DoH clients; without it a random-subdomain flood through the open endpoint would get this host blocklisted by the victim's authoritative servers |
| fail2ban jail for DoH abusers | `modules/nginx.nix` | `nginx-doh-abuse` uses fail2ban's stock `nginx-limit-req` filter parameterised to zone `doh`, reading `error.log`: 20 rejections in 600 s earn an hour, escalated by the global `bantime.increment`. Scoped to `port = "https"` deliberately, unlike the sibling nginx jails that ban every port, because a false positive on an open resolver costs a real client its DNS; RFC1918 is already in `ignoreIP` (`modules/network/firewall.nix`), so LAN and VPN clients cannot ban themselves. Verified with `fail2ban-regex` against the live log: 96 of 1038 lines matched. The matching journal-volume guard lives on the DNS units themselves — `LogRateLimitBurst = 3000` per 30 s in `modules/unbound.nix`, so per-query logging cannot evict sshd or fail2ban history from the 1 GB journal |
| Ad and tracker filtering, blocklist refresh | `modules/unbound.nix` | Two `rpz:` zones scoped by `access-control-tag` to `192.168.1.0/24` and `10.8.0.0/24` only, so the deliberately open DoH endpoint keeps answering honestly instead of lying to strangers. `respip` must lead `module-config` or the clauses are silently inert, and `interface-tag` cannot be used for the scoping: the manual's rule that any `access-control*:` overrides all `interface-*:` was confirmed by test. `rpz.local.` is listed first and generated into the store from Nix allow/block lists, because a `passthru` in the first zone beats a block in the second — that is both the false-positive exception mechanism and where `adfox.ru` and `vk-portal.net` are added, the Russian trackers no HaGeZi tier covers. `rpz.ads.` is HaGeZi Pro refreshed by `unbound-rpz-update.timer`, not by unbound's own `url:`, so a bad publish is rejected before activation: SOA in the header, at least 300 000 rules, and no canary blocked along its whole suffix chain (the eight work portals plus `labile.cc`; upstream does write `work-parent.example` rules — `work-analytics.example` is blocked). Failure keeps the old zone. Costs 285 MB resident against 33 MB before, and ~2 s of startup parsing; `unbound-control rpz_disable rpz.ads.` is the no-rebuild kill switch |
