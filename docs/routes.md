...
| Swap: zram hot tier, NVMe overflow tier, `vm` sysctls (imported by `hosts/configuration.nix` and `hosts/configuration-server.nix`) | `modules/swap.nix` | The 32 GiB box ran with no swap at all and took 70 kernel OOM kills in 9 days of uptime (steamwebhelper, chrome, electron): `pgsteal_anon` was 0 while 72M file pages refaulted, so every byte of reclaim pressure landed on the page cache. `zram0` is `zstd` sized at 50% of RAM, `swap-priority` 100; `/swapfile`, 32 GiB on the root NVMe (`/dev/nvme0n1p2`), is the overflow tier at priority 10 and is created once by `mkswap-swapfile.service` (`dd`, not `fallocate`). The file must never sit on the ZFS pool that `/drive` is becoming: writeback under memory pressure on a ZFS dataset or zvol can need memory to free memory, a known deadlock (openzfs/zfs#7734, open since 2018), so `/swapfile` was placed directly on the ext4 root rather than under `/var/lib`, where `modules/chromadb.nix` already shows the precedent of bind-mounting a subtree onto `/drive` — a path outside `/var/lib` cannot be shadowed by that pattern being extended later. Server previously ran with the file at `/drive/swapfile`: `hosts/configuration-server.nix` (`server: move vm state and swap to sata ssd`, 2026-09-04) carried an `lib.mkForce` override that pointed it at the SATA disk to spare NVMe write endurance, silently diverging from this module's own `/var/lib/swapfile` declaration; that override has been removed. Any live host still carrying a `swapfile` at the old path (`/var/lib/swapfile` or `/drive/swapfile`) needs it deleted by hand with `swapoff` first — the module cannot do this because it does not manage pre-existing files outside its own declared device. On the `pc` host, `zswap.enabled=0` because the CachyOS kernel ships `CONFIG_ZSWAP_DEFAULT_ON` and zswap in front of a zram device compresses every page twice. On the `server` host running `pkgs.linuxPackages`, the kernel has `ZSWAP = option yes` with no `ZSWAP_DEFAULT_ON`, making the `zswap.enabled` parameter inert. `vm.swappiness` needs `lib.mkForce`: musnix (`audio.lowLatency`) defines it as 10 without `mkDefault`, and 10 keeps the kernel from using zram at all. `vm.watermark_scale_factor` 125 keeps kswapd reclaim tighter; `vm.watermark_min_kbytes` is not set. |
