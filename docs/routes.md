...
| SMARTctl exporter device pinning (`services.prometheus.exporters.smartctl.devices`, dashboard `group_left` join) | `modules/grafana.nix`; rationale: `docs/smartctl-exporter.md` | Devices are pinned to `/dev/disk/by-id/...` paths rather than left to autodiscovery, because `/dev/sdX` letters are not stable across reboots on this host and a letter collision during the staleness window breaks every dashboard panel's `group_left` join — see the doc for the exact Prometheus error and the tradeoff (autodiscovery off means a newly added drive needs its by-id path added here manually or it goes unmonitored) |
