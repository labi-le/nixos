...
| Drive mounts (`/drive` 2 TB SATA SSD, `/backup` 4 TB SATA HDD) | `modules/drive.nix` | `/drive` must never carry `_netdev`: it is a local SATA disk, but `_netdev` orders its mount after `network.target`, and `modules/chromadb.nix` bind-mounts a subdirectory of it into `local-fs.target`, closing a cycle through `sockets.target`. This cycle has been present on every boot since the chromadb bind mount landed (`chromadb: keep index off the system nvme`, 2026-09-04) — it is not specific to kernel 6.18.41. systemd breaks it by deleting an arbitrary job, and which job varies by boot: on 2026-09-21 it deleted `nix-daemon.socket`, leaving that boot without a nix daemon; earlier boots (kernel 7.1.5) lost `systemd-update-utmp`, `dbus-broker`, `systemd-tmpfiles-setup` and `drive-swapfile.swap` instead. Verbatim cycle from the 2026-09-21 boot: `sockets.target: Found ordering cycle: nix-daemon.socket/start after sysinit.target/start after systemd-update-done.service/start after systemd-journal-catalog-update.service/start after local-fs.target/start after var-lib-private-chromadb.mount/start after drive.mount/start after network.target/start after network-pre.target/start after resolvconf.service/start after basic.target/start after sockets.target` / `Job nix-daemon.socket/start deleted to break ordering cycle starting with sockets.target/start` |
...
| ZFS support (kernel module, hostId; scrub timer off until pool exists, trim timer on) | `modules/zfs.nix` | Declares `zfs.{enable,hostId,autoScrub,trim}`; `hostId` has no default, so a host must supply it itself — `hosts/configuration-server.nix` sets `zfs.enable = true; zfs.hostId = "204cf2f5";`. That value is a literal pinned to the first 8 hex characters of this host's /etc/machine-id and must never change once a pool exists; it was reverted from reading it with `builtins.readFile` at evaluation time because that broke pure evaluation (making `nixosConfigurations.server` unevaluable with `error: access to absolute path '/etc/machine-id' is forbidden in pure evaluation mode`) and tied the hostId to the building machine instead of the repository. `autoScrub` defaults to false because with no pool `zfs-scrub.service` runs `zpool scrub -w $(zpool list -H -o name) \|\| true`; enable scrubbing only after a pool exists, per `services.zfs.autoScrub.enable = true`. `trim` via `zfs-trim-{timer,service}` auto-discovers SSDs and runs `zpool trim` on them. Both systemd units require `zfs.enable` but neither requires the other. `hostId` has no default for 20-character collision safety: a pool must never move to a different hostId, or its root dataset's `canmount` becomes `no`, leaving mounted children orphaned — this is ZFS's corruption safeguard for multi-host writes (`zfs.hostId ≠ original pool hostId`). Rollback through snapshots is bulletproof; rollback through generations (NixOS `boot.loader.grub.configurationLimit`) is safe — either way, corruption can't hide new datasets and they surface immediately.
