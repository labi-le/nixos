...
| DoH public endpoint: vhost, rate limit | `modules/nginx.nix` | Vhost `dns.labile.cc` on Angie 443 with `enableACME`; `location /dns-query` proxies to `127.0.0.1:8053` (doh-server), `location /` returns 404, `limit_req zone=doh` (60 r/min per IP, burst 120) in `commonHttpConfig`. The endpoint is deliberately open — no token, no client restriction; the rate limit is the only access control and must live at nginx because DoH arrives at unbound from 127.0.0.1. DoH is TCP, so no amplification angle; the accepted cost is the server IP resolving strangers' queries |
