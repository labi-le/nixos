...
| fail2ban: sshd blindness, filter traps, port scoping, mailcow chain conflict | `modules/network/firewall.nix`, `modules/frp.nix`, `docs/fail2ban.md` | The gotchas, rejected alternatives, measured numbers and verification commands that neither module's own row states — the sshd jail's silent OpenSSH 9.8 `sshd-session` blind spot, the rejected Cloudflare real-IP rewrite, why the Cloudflare/mailcow exemption is per jail rather than `[DEFAULT]`, the two nginx filter anchoring traps, and mailcow's `netfilter` container fighting fail2ban for the `INPUT` chain — are in `docs/fail2ban.md`; the nginx-specific jail rationale stays on this table's `modules/nginx.nix` row |
