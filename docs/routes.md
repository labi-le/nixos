...
| Grafana notification channels (two Telegram contact points + agenix secret) | `modules/monitoring/contact-points.nix` | One contact point per chat, never two receivers inside one: Grafana fans a contact point out to *every* receiver it holds, so the `telegram-alt` receiver that commit `e428947` added for frp silently duplicated every other alert into that chat as well. `telegram` is the main chat, `telegram-frp` is frp's only; both share one message template. The template prints `{{ .Annotations.Error }}` because Grafana's `execErrState = "Error"` path builds a `DatasourceError` instance carrying the rule's annotations but none of the query labels, which rendered as a summary with an empty body and no hint that the cause was an evaluation failure |
