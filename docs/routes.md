...
| Nginx; aborted HTTP/2 downloads, KTLS; fail2ban nginx jails (`nginx-botsearch`, `nginx-bad-request`, `nginx-scan-404`, `nginx-secret-probe`) and their filters, all four scoped to `port = "http,https,38264"` (nginx's three listeners — 80, 443, and the `93.100.194.40` vhost's 38264 — so an HTTP ban does not also take ssh/DoT/NFS with it but does cover every port nginx itself answers on) rather than relying on `services.fail2ban.ignoreIP`, since a jail-level `ignoreip` replaces rather than merges with `[DEFAULT]` and this exemption must not reach the `sshd` jail; `nginx-botsearch`, `nginx-bad-request` and `nginx-scan-404` set their own `ignoreip` (RFC1918 plus Cloudflare's `104.16.0.0/12` ARIN allocation and its other published proxy ranges, covering the 104.28.x WARP exits, for shared exits carrying legitimate radio listeners), but `nginx-secret-probe` deliberately carries no `ignoreip` at all: its failregex only matches credential-file and `/wp-admin/`/`/wp-login`/`/phpmyadmin` probe shapes that no legitimate listener traffic can produce, so the Cloudflare exemption there would only make real probes unbannable | `modules/nginx.nix`, `docs/nginx.md` |
