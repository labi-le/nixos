...
| LLM gateway API key for omp hosts (agenix `opencode-litellm-master-key`, consumed by both `home-manager/modules/omp.nix` and the server's `modules/omp.nix`) | `modules/opencode-secrets.nix` | Enabled for pc, server, notebook (hosts listed in the module) |
...
| Oh My Pi (omp) coding agent — server copy; standalone NixOS duplicate of `home-manager/modules/omp.nix` (the server runs `withHomeManager = false`, so the HM module is unavailable there), gated by `mySystem.omp.enable`; agent-context payload (`AGENTS.md`, `RULES.md`, `rules/`, `extensions/`, vendored skills) is a deliberate copy of `home-manager/modules/omp/` under `modules/omp/` | `modules/omp.nix` | read-only files (`models.yml`, `mcp.json`, `lsp.json`, `keybindings.yml`, `AGENTS.md`, `RULES.md`, `rules/`, `extensions/`, vendored skills) deploy as `systemd.tmpfiles.rules` `L+` symlinks into `~/.omp/agent/`; `config.yml` is the exception — a writable regular file (mode 600) written by `system.activationScripts.ompConfig`, because omp rewrites it at runtime under a lock and a store symlink fails with EACCES |
...
| System service | `flake.nix` (`mkSystem` `withHomeManager` list) / `modules/omp.nix` (server) | HM hosts get it from the `mkSystem` list: `withHomeManager` imports `inputs.index-repo.nixosModules.default` and sets `services.index-repo.enable = true`. The server gets it from `modules/omp.nix`, which imports the module itself and sets `services.index-repo.{enable,host,package}` |
| Oh-my-pi register hook | `home-manager/modules/omp.nix` (HM) / `modules/omp.nix` (server) | HM: sets `services.index-repo.omp.registerHook.enable` (an option the NixOS module does not have). Server: deploys upstream `hooks/omp/repo-register.js` itself — an `L+` symlink at `~/.omp/agent/extensions/repo-register.js` to a store copy with `@index_repo_bin@` replaced by `${pkgs.index-repo}/bin/index-repo` |
